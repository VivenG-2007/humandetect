"""
Human Detection + Filter + Face Effects Webcam App  ── OPTIMISED v3.0
======================================================================
Key improvements over v2.0
  • Threaded YOLO  – inference runs in a background thread; camera/UI never block
  • Frame skipping  – YOLO runs every other frame; last mask is reused in between
  • Reduced inference size – YOLO sees 480-px-wide frames (≈3× faster than 1280)
  • Vectorised numpy  – hot-path loops replaced with array ops
  • Full-body detection – segmentation mask used for skeleton / stick-figure joints
    (not just bounding-box heuristics)
  • Mask smoothing kept but cheaper (single GaussianBlur instead of morph+blur)
  • Background cache keyed correctly (no repeated realloc)
  • FaceDetector made optional / lazy (doesn't stall startup)

Install:
    pip install ultralytics opencv-python numpy

Controls (keyboard + click-able left panel):
  ─── BODY FILTERS ──── ─── FACE EFFECTS ───────────
  1 Raw        6 Cyberpunk   E Elf Ears    M Mermaid
  2 Neon       7 Pixel Art   V Vampire     F Fairy
  3 Cartoon    8 Oil Paint   A Angel       Y Baby
  4 Anime      9 Heat        D Demon       C Child
  5 Sketch     0 Glitch      T Teen        O Old Age
  R Skeleton   X Stick       P Age Prog
  U Bubbles
  B Cycle BG   K Toggle boxes   S Save   Q Quit
"""

import cv2
import numpy as np
import time
import os
import math
import threading
import warnings
warnings.filterwarnings("ignore")
os.environ["YOLO_VERBOSE"] = "False"

from ultralytics import YOLO

# ─────────────────────────────────────────────────────────────────────────────
# CONSTANTS
# ─────────────────────────────────────────────────────────────────────────────
PANEL_W   = 230
BTN_H     = 32
SECTION_H = 24
PAD       = 6

# BGR palette
C_BG          = (14,  14,  22)
C_SECTION_BG  = (26,  26,  44)
C_BTN         = (32,  32,  52)
C_BTN_ACTIVE  = (0,  210, 130)
C_BTN_ACTIVE2 = (0,  170, 100)
C_BORDER      = (50,  50,  80)
C_BORDER_ACT  = (0,  240, 150)
C_ACTIVE_TEXT = (8,    8,   8)
C_TEXT        = (185, 185, 208)
C_SECTION_TEXT= (0,  220, 180)
C_ACCENT      = (0,  180, 255)
C_NEW_BADGE   = (0,  140, 255)
CAT_BODY      = (0,  200, 140)
CAT_NEW       = (0,  160, 255)
CAT_FACE      = (200, 80, 220)
CAT_BG        = (220, 140, 20)
C_BOX_BTN_ON  = (20, 130,  20)
C_BOX_BTN_OFF = (55,  55,  80)

FONT    = cv2.FONT_HERSHEY_SIMPLEX
FS      = 0.37
FS_SEC  = 0.40
FS_BADGE= 0.28

# YOLO inference width (smaller = faster; mask is up-scaled back)
INFER_W = 480

# ─────────────────────────────────────────────────────────────────────────────
# CAMERA FINDER
# ─────────────────────────────────────────────────────────────────────────────

def find_camera(max_index=5):
    print("Scanning for cameras...")
    found = []
    for i in range(max_index + 1):
        try:
            cap = cv2.VideoCapture(i, cv2.CAP_DSHOW)
            if not cap.isOpened():
                cap = cv2.VideoCapture(i)
            if cap.isOpened():
                ret, _ = cap.read()
                if ret:
                    found.append(i)
                    print(f"  [OK] Camera index {i}")
            cap.release()
        except Exception:
            pass
    return found

def open_camera(index):
    cap = cv2.VideoCapture(index, cv2.CAP_DSHOW)
    if not cap.isOpened():
        cap = cv2.VideoCapture(index)
    return cap

# ─────────────────────────────────────────────────────────────────────────────
# THREADED YOLO SEGMENTOR
# ─────────────────────────────────────────────────────────────────────────────

class YOLOSegmentor:
    """
    Runs YOLOv8-seg in a background thread.
    The main thread always gets the latest completed mask instantly.
    """
    def __init__(self, model_name="yolov8n-seg.pt"):
        print(f"Loading {model_name} …")
        self.model  = YOLO(model_name)
        # Warm-up pass so first real frame is fast
        dummy = np.zeros((480, 480, 3), dtype=np.uint8)
        self.model(dummy, classes=[0], verbose=False, imgsz=320)
        print("Model ready.")

        self._lock        = threading.Lock()
        self._frame_in    = None   # latest frame queued for inference
        self._mask_out    = None   # latest completed mask
        self._boxes_out   = []
        self._running     = True
        self._new_frame   = threading.Event()

        self._thread = threading.Thread(target=self._worker, daemon=True)
        self._thread.start()

    def _worker(self):
        while self._running:
            self._new_frame.wait(timeout=0.5)
            self._new_frame.clear()
            with self._lock:
                frame = self._frame_in
            if frame is None:
                continue

            h_orig, w_orig = frame.shape[:2]
            # Downscale for inference
            scale  = INFER_W / w_orig
            infer_h = int(h_orig * scale)
            small  = cv2.resize(frame, (INFER_W, infer_h))

            mask  = np.zeros((h_orig, w_orig), dtype=np.uint8)
            boxes = []

            results = self.model(small, classes=[0], verbose=False,
                                 imgsz=INFER_W, half=False)
            r = results[0]
            if r.masks is not None:
                for seg in r.masks.data:
                    m = seg.cpu().numpy()
                    # Resize back to original resolution
                    m = cv2.resize(m, (w_orig, h_orig))
                    m = (m > 0.45).astype(np.uint8) * 255
                    mask = cv2.bitwise_or(mask, m)
            if r.boxes is not None:
                for box in r.boxes.xyxy.cpu().numpy():
                    # Scale boxes back to original resolution
                    b = (box / scale).astype(int)
                    boxes.append(b)

            # Single cheap smoothing pass
            mask = cv2.GaussianBlur(mask, (15, 15), 0)

            with self._lock:
                self._mask_out  = mask
                self._boxes_out = boxes

    def submit(self, frame):
        """Queue a frame for inference (non-blocking)."""
        with self._lock:
            self._frame_in = frame
        self._new_frame.set()

    def get_result(self):
        """Return (mask, boxes) — latest completed inference (may be 1 frame old)."""
        with self._lock:
            mask  = self._mask_out
            boxes = list(self._boxes_out)
        if mask is None:
            h, w = 480, 640
            mask = np.zeros((h, w), dtype=np.uint8)
        return mask, boxes

    def stop(self):
        self._running = False
        self._new_frame.set()

# ─────────────────────────────────────────────────────────────────────────────
# FACE DETECTOR (lazy, runs in main thread — fast Haar)
# ─────────────────────────────────────────────────────────────────────────────

class FaceDetector:
    def __init__(self):
        self.face_cascade = cv2.CascadeClassifier(
            cv2.data.haarcascades + "haarcascade_frontalface_default.xml")

    def detect(self, frame):
        gray  = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        gray  = cv2.equalizeHist(gray)
        faces = self.face_cascade.detectMultiScale(
            gray, scaleFactor=1.1, minNeighbors=5, minSize=(60, 60))
        return faces if len(faces) > 0 else []

# ─────────────────────────────────────────────────────────────────────────────
# UTILITY
# ─────────────────────────────────────────────────────────────────────────────

def m3f(mask):
    return cv2.cvtColor(mask, cv2.COLOR_GRAY2BGR).astype(np.float32) / 255.0

def alpha_blend(bg, fg, alpha3):
    """alpha3 is float32 [0,1] HxWx3."""
    return np.clip(fg * alpha3 + bg * (1.0 - alpha3), 0, 255).astype(np.uint8)

# ─────────────────────────────────────────────────────────────────────────────
# FULL-BODY JOINT ESTIMATION (uses segmentation mask contour)
# ─────────────────────────────────────────────────────────────────────────────

def estimate_joints_from_mask(mask_roi, ox, oy):
    """
    Derive joint positions from the *actual* silhouette of the person
    rather than pure bounding-box ratios.  Returns (joints, bones, head_c, head_r).
    mask_roi: uint8 single-channel crop of the person mask.
    ox, oy  : offset of the crop in the full frame.
    """
    h, w = mask_roi.shape[:2]
    if h < 50 or w < 20:
        return [], [], (ox + w//2, oy), max(8, w//8)

    # Horizontal profile — width of mask at each row (used for body width estimate)
    horiz = (mask_roi > 127).astype(np.uint8)
    row_sum = horiz.sum(axis=1).astype(np.float32)  # pixels per row

    # Find topmost / bottommost occupied rows
    occupied = np.where(row_sum > w * 0.05)[0]
    if len(occupied) < 10:
        top, bot = 0, h - 1
    else:
        top, bot = int(occupied[0]), int(occupied[-1])
    body_h = max(1, bot - top)

    # Head top / torso split by scanning from top
    # Head ends roughly where mask width starts expanding (shoulders)
    head_top  = top
    mid_h     = (top + bot) // 2

    def col_centre(row_frac):
        """Horizontal centroid of mask at a given fraction of body height."""
        row = int(top + row_frac * body_h)
        row = min(max(row, 0), h - 1)
        cols = np.where(horiz[row] > 0)[0]
        if len(cols) == 0:
            return ox + w // 2
        return int(ox + (cols[0] + cols[-1]) // 2)

    def row_width(row_frac):
        row = int(top + row_frac * body_h)
        row = min(max(row, 0), h - 1)
        return int(row_sum[row])

    cx_body = col_centre(0.30)

    # Head
    head_r   = max(10, int(row_width(0.05) * 0.55))
    head_cy  = oy + int(top + body_h * 0.07)
    head_c   = (col_centre(0.07), head_cy)

    neck     = (col_centre(0.18), oy + int(top + body_h * 0.18))
    spine_m  = (col_centre(0.38), oy + int(top + body_h * 0.38))
    pelvis   = (col_centre(0.52), oy + int(top + body_h * 0.54))

    # Shoulder width estimate from mask width at 25 %
    sh_w = row_width(0.25)
    sh_half = max(8, sh_w // 2)
    sh_cy = oy + int(top + body_h * 0.25)
    l_sho = (col_centre(0.25) - sh_half // 2, sh_cy)
    r_sho = (col_centre(0.25) + sh_half // 2, sh_cy)

    # Arms: extend beyond shoulder width
    elb_dy  = oy + int(top + body_h * 0.43)
    wri_dy  = oy + int(top + body_h * 0.61)
    arm_ext = max(10, sh_half // 2)
    l_elb = (l_sho[0] - arm_ext, elb_dy)
    r_elb = (r_sho[0] + arm_ext, elb_dy)
    l_wri = (l_elb[0] - arm_ext // 2, wri_dy)
    r_wri = (r_elb[0] + arm_ext // 2, wri_dy)

    # Hips
    hip_w = row_width(0.54)
    hip_half = max(8, hip_w // 2)
    hip_cy = oy + int(top + body_h * 0.54)
    l_hip = (col_centre(0.54) - hip_half // 2, hip_cy)
    r_hip = (col_centre(0.54) + hip_half // 2, hip_cy)

    kne_cy = oy + int(top + body_h * 0.73)
    ank_cy = oy + int(top + body_h * 0.93)
    leg_sep = max(6, hip_half // 2)
    l_kne = (l_hip[0] - leg_sep // 4, kne_cy)
    r_kne = (r_hip[0] + leg_sep // 4, kne_cy)
    l_ank = (l_kne[0],  ank_cy)
    r_ank = (r_kne[0],  ank_cy)

    joints = [head_c, neck, l_sho, r_sho, spine_m, pelvis,
              l_elb, r_elb, l_wri, r_wri,
              l_hip, r_hip, l_kne, r_kne, l_ank, r_ank]

    bones = [
        (neck, head_c),
        (neck, spine_m),
        (spine_m, pelvis),
        (neck, l_sho), (l_sho, l_elb), (l_elb, l_wri),
        (neck, r_sho), (r_sho, r_elb), (r_elb, r_wri),
        (pelvis, l_hip), (l_hip, l_kne), (l_kne, l_ank),
        (pelvis, r_hip), (r_hip, r_kne), (r_kne, r_ank),
    ]

    return joints, bones, head_c, head_r

# ─────────────────────────────────────────────────────────────────────────────
# FACE / CHARACTER EFFECT FUNCTIONS  (unchanged logic, slightly cleaner)
# ─────────────────────────────────────────────────────────────────────────────

def effect_elf_ears(frame, faces, t):
    out = frame.copy()
    for (fx, fy, fw, fh) in faces:
        ew = fw // 5; eh = fw // 3
        for side, lx in [(-1, fx - ew//2), (1, fx + fw - ew//2)]:
            pts = np.array([[lx, fy+eh],[lx+ew//2, fy-eh+eh//6],[lx+ew, fy+eh]], np.int32)
            cv2.fillPoly(out, [pts], (130,190,235))
            cv2.polylines(out, [pts], True, (60,110,160), 2)
            inner = pts.copy(); inner[:,0] += side*ew//5; inner[:,1] += eh//5
            cv2.polylines(out, [inner], False, (100,150,200), 1)
    return out

def effect_vampire_teeth(frame, faces, t):
    out = frame.copy()
    for (fx, fy, fw, fh) in faces:
        mx=fx+fw//2; my=fy+int(fh*0.75); tw=fw//10; th=fw//8; gap=fw//16
        for side in [-1,1]:
            tx=mx+side*gap-tw//2; ty=my
            pts=np.array([[tx,ty],[tx+tw,ty],[tx+tw//2+2,ty+th],[tx+tw//2-2,ty+th]],np.int32)
            cv2.fillPoly(out,[pts],(245,245,255))
            drip=int(4+4*abs(math.sin(t*1.5)))
            cv2.line(out,(tx+tw//2,ty+th),(tx+tw//2,ty+th+drip),(0,0,200),2)
            cv2.circle(out,(tx+tw//2,ty+th+drip),3,(0,0,180),-1)
    return out

def effect_angel(frame, faces, t):
    out = frame.copy()
    for (fx, fy, fw, fh) in faces:
        cx=fx+fw//2
        halo_y=fy-fw//6; halo_rx=fw//3; halo_ry=fw//10
        pulse=1.0+0.04*math.sin(t*3); halo_rx_p=int(halo_rx*pulse)
        for thickness,alpha in [(20,40),(12,80),(6,160),(3,255)]:
            ov=out.copy(); cv2.ellipse(ov,(cx,halo_y),(halo_rx_p,halo_ry),0,0,360,(30,200,255),thickness)
            out=cv2.addWeighted(out,1-alpha/255,ov,alpha/255,0)
        wing_w=fw*2; wing_h=int(fh*1.6); wing_y=fy+fh//3
        for side in [-1,1]:
            pts_outer=[]
            for ang in range(0,181,8):
                rad=math.radians(ang)
                ex=cx+side*int(wing_w*math.sin(rad)); ey=wing_y-int(wing_h*0.5*math.sin(rad*0.5))
                pts_outer.append([ex,ey])
            ov=out.copy(); cv2.fillPoly(ov,[np.array(pts_outer,np.int32)],(240,240,255))
            out=cv2.addWeighted(out,0.45,ov,0.55,0)
    return out

def effect_demon_horns(frame, faces, t):
    out = frame.copy()
    for (fx, fy, fw, fh) in faces:
        cx=fx+fw//2; horn_h=int(fw*0.5); horn_w=fw//7
        for side in [-1,1]:
            hx=cx+side*fw//4; hy=fy; sway=int(4*math.sin(t*2))
            pts=np.array([[hx-horn_w//2,hy],[hx+side*sway-2,hy-horn_h],[hx+horn_w//2,hy]],np.int32)
            shadow=pts.copy(); shadow[:,0]+=3; shadow[:,1]+=3
            cv2.fillPoly(out,[shadow],(0,0,30)); cv2.fillPoly(out,[pts],(20,20,160))
            cv2.polylines(out,[pts],True,(0,0,80),2)
        ley=fy+int(fh*0.38); lex=fx+int(fw*0.28); rex=fx+int(fw*0.72); glow_r=int(fw*0.07)
        for eye_cx,eye_cy in [(lex,ley),(rex,ley)]:
            ov=out.copy(); cv2.circle(ov,(eye_cx,eye_cy),glow_r+6,(0,0,255),-1)
            out=cv2.addWeighted(out,0.6,ov,0.4,0)
            cv2.circle(out,(eye_cx,eye_cy),glow_r,(0,80,255),-1)
    return out

def effect_mermaid(frame, faces, t):
    out=frame.copy(); h_f,w_f=frame.shape[:2]
    tint=np.zeros_like(out,dtype=np.float32); tint[:,:,0]=60; tint[:,:,1]=40
    out=np.clip(out.astype(np.float32)*0.80+tint,0,255).astype(np.uint8)
    for (fx,fy,fw,fh) in faces:
        cx=fx+fw//2; scale_r=max(1,fw//14)
        for row in range(8):
            for col in range(12):
                sx=fx+col*scale_r*2-scale_r; sy=fy+row*scale_r+(scale_r if col%2 else 0)
                if fx<=sx<=fx+fw and fy<=sy<=fy+fh:
                    hue=int((row*30+col*15+t*40)%180)
                    col_bgr=cv2.cvtColor(np.uint8([[[hue,200,200]]]),cv2.COLOR_HSV2BGR)[0][0]
                    ov=out.copy()
                    cv2.ellipse(ov,(sx,sy),(scale_r,max(1,scale_r-1)),0,0,360,
                                (int(col_bgr[0]),int(col_bgr[1]),int(col_bgr[2])),-1)
                    out=cv2.addWeighted(out,0.75,ov,0.25,0)
    return out

def effect_fairy(frame, faces, t):
    out=frame.copy(); h_f,w_f=frame.shape[:2]; rng=np.random.default_rng(7)
    positions=[(int(rng.integers(0,w_f)),int(rng.integers(0,h_f)),
                rng.uniform(0.5,3.0),rng.uniform(0,2*math.pi)) for _ in range(40)]
    for sx,sy,sp,ph in positions:
        bri=0.5+0.5*math.sin(t*sp+ph); sz=int(1+bri*5)
        ov=out.copy()
        col=(int(180+bri*75),int(180+bri*75),255)
        cv2.line(ov,(sx-sz,sy),(sx+sz,sy),col,1); cv2.line(ov,(sx,sy-sz),(sx,sy+sz),col,1)
        cv2.circle(ov,(sx,sy),max(1,sz//2),(255,255,255),-1)
        out=cv2.addWeighted(out,1-bri/4,ov,bri/4,0)
    return out

def effect_baby_face(frame, faces, t):
    out=frame.copy()
    for (fx,fy,fw,fh) in faces:
        cx=fx+fw//2; cy=fy+fh//2; roi=out[fy:fy+fh,fx:fx+fw]
        if roi.size==0: continue
        big_w=int(fw*1.25); big_h=int(fh*1.15)
        big=cv2.resize(roi,(big_w,big_h))
        bx=max(0,cx-big_w//2); by=max(0,cy-big_h//2)
        bx2=min(out.shape[1],bx+big_w); by2=min(out.shape[0],by+big_h)
        cw=bx2-bx; ch=by2-by
        if cw<=0 or ch<=0: continue
        resized=cv2.resize(big,(cw,ch))
        pm=np.zeros((ch,cw),dtype=np.uint8)
        cv2.ellipse(pm,(cw//2,ch//2),(cw//2-2,ch//2-2),0,0,360,255,-1)
        pm=cv2.GaussianBlur(pm,(21,21),0)
        a3=np.stack([pm/255.]*3,axis=2)
        out[by:by2,bx:bx2]=np.clip(resized*a3+out[by:by2,bx:bx2]*(1-a3),0,255).astype(np.uint8)
        for side in [-1,1]:
            chx=cx+side*int(fw*0.32); chy=fy+int(fh*0.62)
            ov=out.copy(); cv2.circle(ov,(chx,chy),fw//6,(100,130,220),-1)
            out=cv2.addWeighted(out,0.75,ov,0.25,0)
    return out

def effect_child(frame, faces, t):
    out=frame.copy()
    for (fx,fy,fw,fh) in faces:
        roi=out[fy:fy+fh,fx:fx+fw]
        if roi.size==0: continue
        smooth=cv2.bilateralFilter(roi,9,60,60)
        hsv=cv2.cvtColor(smooth,cv2.COLOR_BGR2HSV).astype(np.float32)
        hsv[:,:,1]=np.clip(hsv[:,:,1]*0.80,0,255); hsv[:,:,2]=np.clip(hsv[:,:,2]*1.15,0,255)
        out[fy:fy+fh,fx:fx+fw]=cv2.cvtColor(hsv.astype(np.uint8),cv2.COLOR_HSV2BGR)
    return out

def effect_teen(frame, faces, t):
    out=frame.copy()
    for (fx,fy,fw,fh) in faces:
        roi=out[fy:fy+fh,fx:fx+fw].astype(np.float32)
        if roi.size==0: continue
        k=np.array([[0,-0.3,0],[-0.3,2.2,-0.3],[0,-0.3,0]])
        sharp=cv2.filter2D(roi,-1,k)
        sharp[:,:,0]=np.clip(sharp[:,:,0]*1.1,0,255)
        sharp[:,:,1]=np.clip(sharp[:,:,1]*1.05,0,255)
        sharp[:,:,2]=np.clip(sharp[:,:,2]*0.93,0,255)
        out[fy:fy+fh,fx:fx+fw]=sharp.astype(np.uint8)
    return out

def effect_old_age(frame, faces, t):
    out=frame.copy()
    for (fx,fy,fw,fh) in faces:
        roi=out[fy:fy+fh,fx:fx+fw].copy()
        if roi.size==0: continue
        hsv=cv2.cvtColor(roi,cv2.COLOR_BGR2HSV).astype(np.float32)
        hsv[:,:,1]=np.clip(hsv[:,:,1]*0.40,0,255); hsv[:,:,2]=np.clip(hsv[:,:,2]*0.90,0,255)
        roi=cv2.cvtColor(hsv.astype(np.uint8),cv2.COLOR_HSV2BGR)
        noise=np.random.normal(0,6,roi.shape).astype(np.int16)
        roi=np.clip(roi.astype(np.int16)+noise,0,255).astype(np.uint8)
        out[fy:fy+fh,fx:fx+fw]=roi
    return out

AGE_PROGRESSION_STAGE = [0]

def effect_age_progression(frame, faces, t):
    stage=AGE_PROGRESSION_STAGE[0]
    funcs=[effect_baby_face,effect_child,effect_teen,lambda f,fa,t_: f.copy(),effect_old_age]
    names=["Baby","Child","Teen","Adult","Elderly"]
    out=funcs[stage](frame,faces,t)
    h_f=frame.shape[0]
    label=f"Age Stage: {names[stage]}"
    cv2.rectangle(out,(0,h_f-55),(len(label)*11+20,h_f-32),(0,0,0),-1)
    cv2.putText(out,label,(10,h_f-38),FONT,0.55,(0,230,180),1)
    return out

FACE_EFFECTS = {
    ord('e'):("Elf Ears",      effect_elf_ears),
    ord('v'):("Vampire Teeth", effect_vampire_teeth),
    ord('a'):("Angel Wings",   effect_angel),
    ord('d'):("Demon Horns",   effect_demon_horns),
    ord('m'):("Mermaid",       effect_mermaid),
    ord('f'):("Fairy Sparkles",effect_fairy),
    ord('y'):("Baby Face",     effect_baby_face),
    ord('c'):("Child Look",    effect_child),
    ord('t'):("Teen Look",     effect_teen),
    ord('o'):("Old Age",       effect_old_age),
    ord('p'):("Age Progress",  effect_age_progression),
}

# ─────────────────────────────────────────────────────────────────────────────
# BODY FILTERS  (vectorised hot paths)
# ─────────────────────────────────────────────────────────────────────────────

def f_raw(frame, mask): return frame.copy()

def f_neon_outline(frame, mask):
    dark=(frame * 0.12).astype(np.uint8)
    hard=(mask > 127).astype(np.uint8)*255
    outline=cv2.Canny(hard,30,100)
    outline=cv2.dilate(outline,np.ones((3,3),np.uint8),iterations=2)
    gw=cv2.GaussianBlur(outline,(21,21),0)
    gn=cv2.GaussianBlur(outline,(7,7),0)
    neon=np.zeros((*frame.shape[:2],3),dtype=np.float32)
    neon[:,:,0]+=gw.astype(np.float32)*1.2+gn.astype(np.float32)*1.5
    neon[:,:,1]+=gw.astype(np.float32)*1.4+gn.astype(np.float32)*1.5
    neon[:,:,2]+=gw.astype(np.float32)*0.3+gn.astype(np.float32)*1.5
    neon=np.clip(neon,0,255).astype(np.uint8)
    alpha=m3f(mask)
    body=(frame.astype(np.float32)*alpha*0.35+dark.astype(np.float32)*(1-alpha))
    return np.clip(body,0,255).astype(np.uint8)+neon

def f_cartoon(frame, mask):
    data=frame.reshape((-1,3)).astype(np.float32)
    _,labels,centers=cv2.kmeans(data,8,None,
        (cv2.TERM_CRITERIA_EPS+cv2.TERM_CRITERIA_MAX_ITER,8,1.0),
        3,cv2.KMEANS_RANDOM_CENTERS)
    quant=np.uint8(centers)[labels.flatten()].reshape(frame.shape)
    gray=cv2.medianBlur(cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY),7)
    edges=cv2.adaptiveThreshold(gray,255,cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY,9,6)
    cartoon=cv2.bitwise_and(quant,cv2.cvtColor(edges,cv2.COLOR_GRAY2BGR))
    hsv=cv2.cvtColor(cartoon,cv2.COLOR_BGR2HSV).astype(np.float32)
    hsv[:,:,1]=np.clip(hsv[:,:,1]*1.8,0,255)
    return cv2.cvtColor(hsv.astype(np.uint8),cv2.COLOR_HSV2BGR)

def f_anime(frame, mask):
    s=frame.copy()
    for _ in range(3): s=cv2.bilateralFilter(s,9,75,75)  # was 5 passes
    gray=cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
    edges=cv2.adaptiveThreshold(cv2.medianBlur(gray,5),255,
        cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY,9,3)
    anime=cv2.bitwise_and(s,cv2.cvtColor(edges,cv2.COLOR_GRAY2BGR))
    hsv=cv2.cvtColor(anime,cv2.COLOR_BGR2HSV).astype(np.float32)
    hsv[:,:,1]=np.clip(hsv[:,:,1]*2.0,0,255)
    return cv2.cvtColor(hsv.astype(np.uint8),cv2.COLOR_HSV2BGR)

def f_pencil_sketch(frame, mask):
    gray=cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
    blur=cv2.GaussianBlur(cv2.bitwise_not(gray),(25,25),0)
    sketch=cv2.divide(gray,cv2.bitwise_not(blur),scale=256.0)
    return cv2.cvtColor(sketch,cv2.COLOR_GRAY2BGR)

def f_cyberpunk(frame, mask):
    dark=(frame*0.08).astype(np.uint8)
    gray=cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
    edges=cv2.dilate(cv2.Canny(gray,30,100),np.ones((2,2),np.uint8))
    eb=cv2.GaussianBlur(edges,(5,5),0).astype(np.float32)
    neon=np.zeros_like(frame,dtype=np.float32)
    neon[:,:,0]=np.roll(eb,5,axis=1)*2.5
    neon[:,:,1]=np.roll(eb,-5,axis=0)*1.5
    neon[:,:,2]=eb*2.0
    neon=np.clip(neon,0,255).astype(np.uint8)
    tinted=frame.astype(np.float32)
    tinted[:,:,0]=np.clip(tinted[:,:,0]*1.4,0,255)
    tinted[:,:,2]=np.clip(tinted[:,:,2]*0.6,0,255)
    alpha=m3f(mask)
    body=np.clip(tinted*alpha*0.6+dark.astype(np.float32)*(1-alpha),0,255).astype(np.uint8)
    return cv2.add(body,neon)

def f_pixel_art(frame, mask):
    h,w=frame.shape[:2]; px=10
    sm=cv2.resize(frame,(w//px,h//px),interpolation=cv2.INTER_LINEAR)
    pix=cv2.resize(sm,(w,h),interpolation=cv2.INTER_NEAREST)
    return (pix//(256//6))*(256//6)

def f_oil_paint(frame, mask):
    oil=frame.copy()
    for _ in range(5): oil=cv2.bilateralFilter(oil,9,150,150)  # was 7
    hsv=cv2.cvtColor(oil,cv2.COLOR_BGR2HSV).astype(np.float32)
    hsv[:,:,1]=np.clip(hsv[:,:,1]*1.3,0,255)
    return cv2.cvtColor(hsv.astype(np.uint8),cv2.COLOR_HSV2BGR)

def f_heat_vision(frame, mask):
    gray=cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
    alpha=mask.astype(np.float32)/255.0
    boosted=np.clip(gray.astype(np.float32)+alpha*60,0,255).astype(np.uint8)
    return cv2.applyColorMap(boosted,cv2.COLORMAP_INFERNO)

def f_glitch(frame, mask):
    h,w=frame.shape[:2]; out=frame.copy(); shift=8
    b,g,r=cv2.split(out)
    out=cv2.merge([np.roll(b,-shift,axis=1),g,np.roll(r,shift,axis=1)])
    rng=np.random.default_rng(int(time.time()*10)%10000)
    for _ in range(6):
        y1=int(rng.integers(0,h)); y2=min(y1+int(rng.integers(2,12)),h)
        out[y1:y2]=np.roll(out[y1:y2],int(rng.integers(-30,30)),axis=1)
    return out

# ─────────────────────────────────────────────────────────────────────────────
# SKELETON FILTER  (uses full mask contours for joint placement)
# ─────────────────────────────────────────────────────────────────────────────

def f_skeleton(frame, mask):
    h, w = frame.shape[:2]
    t    = time.time()
    out  = (frame * 0.08).astype(np.uint8)
    hard = (mask > 127).astype(np.uint8) * 255
    contours, _ = cv2.findContours(hard, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    pulse = 0.85 + 0.15 * math.sin(t * 4)

    for cnt in contours:
        bx, by, bw, bh = cv2.boundingRect(cnt)
        if bw < 30 or bh < 50: continue
        mask_roi = hard[by:by+bh, bx:bx+bw]
        joints, bones, head_c, head_r = estimate_joints_from_mask(mask_roi, bx, by)
        if not joints: continue

        for p1, p2 in bones:
            cv2.line(out, p1, p2, (0, 60, 20), 10, cv2.LINE_AA)
            cv2.line(out, p1, p2, (0, 160, 60), 4, cv2.LINE_AA)
            cv2.line(out, p1, p2, (0, 255, 100), 2, cv2.LINE_AA)

        # Ribcage
        spine_y = joints[4][1]; rib_w = int(bw*0.30); rib_h = int(bh*0.12)
        for i in range(3):
            ry = spine_y - int(rib_h*0.5*i)
            cv2.ellipse(out,(head_c[0],ry),(rib_w,max(4,rib_h-i*2)),
                        0,0,360,(0,100,40),1,cv2.LINE_AA)

        # Skull
        cv2.circle(out, head_c, head_r+6, (0,40,15), -1)
        cv2.circle(out, head_c, head_r,   (0,60,25), -1)
        cv2.circle(out, head_c, head_r,   (0,220,80), 2, cv2.LINE_AA)
        eye_off = int(head_r*0.35); eye_r = int(head_r*0.22)
        for ex_off in [-eye_off, eye_off]:
            ex=head_c[0]+ex_off; ey=head_c[1]+int(head_r*0.1)
            ov=out.copy(); cv2.circle(ov,(ex,ey),eye_r+4,(0,200,60),-1)
            out=cv2.addWeighted(out,1-0.4*pulse,ov,0.4*pulse,0)
            cv2.circle(out,(ex,ey),eye_r,(0,255,120),-1)

        for j in joints[1:]:
            jr=max(4,int(bw*0.025))
            cv2.circle(out,j,jr+4,(0,50,20),-1)
            cv2.circle(out,j,jr,(0,255,100),-1)

    # Scanlines
    out[::3] = np.clip(out[::3].astype(np.int16)+[0,12,4],0,255).astype(np.uint8)
    return out

# ─────────────────────────────────────────────────────────────────────────────
# STICK FIGURE FILTER
# ─────────────────────────────────────────────────────────────────────────────

def f_stick_figure(frame, mask):
    h, w = frame.shape[:2]
    t    = time.time()
    bg_a = (1.0 - mask.astype(np.float32)/255.0)
    bg3  = np.stack([bg_a]*3, axis=2)
    dark = (frame*0.10).astype(np.float32)
    out  = np.clip(frame.astype(np.float32)*bg3 + dark*(1-bg3), 0,255).astype(np.uint8)

    hard = (mask > 127).astype(np.uint8)*255
    contours, _ = cv2.findContours(hard, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    hue = int((t*40)%180)
    stick_col = tuple(int(v) for v in cv2.cvtColor(np.uint8([[[hue,255,255]]]),cv2.COLOR_HSV2BGR)[0][0])
    glow_col  = tuple(max(0,v//3) for v in stick_col)

    def w_pt(p, bw, bh):
        wx = int(p[0] + bw*0.015*math.sin(t*3+p[1]*0.05))
        wy = int(p[1] + bh*0.015*math.cos(t*2.5+p[0]*0.05))
        return (wx, wy)

    for cnt in contours:
        bx, by, bw, bh = cv2.boundingRect(cnt)
        if bw < 30 or bh < 50: continue
        mask_roi = hard[by:by+bh, bx:bx+bw]
        joints, bones, head_c, head_r = estimate_joints_from_mask(mask_roi, bx, by)
        if not joints: continue

        for p1, p2 in bones:
            wp1, wp2 = w_pt(p1,bw,bh), w_pt(p2,bw,bh)
            cv2.line(out,wp1,wp2,glow_col,10,cv2.LINE_AA)
            cv2.line(out,wp1,wp2,stick_col,4,cv2.LINE_AA)
            cv2.line(out,wp1,wp2,(255,255,255),1,cv2.LINE_AA)
        for j in joints[1:]:
            wj=w_pt(j,bw,bh)
            cv2.circle(out,wj,7,glow_col,-1)
            cv2.circle(out,wj,4,stick_col,-1)
        wh=w_pt(head_c,bw,bh)
        cv2.circle(out,wh,head_r+6,glow_col,-1)
        cv2.circle(out,wh,head_r,stick_col,3,cv2.LINE_AA)
        eye_off=int(head_r*0.35); eye_r=max(2,int(head_r*0.12))
        for eo in [-eye_off,eye_off]:
            cv2.circle(out,(wh[0]+eo,wh[1]-int(head_r*0.15)),eye_r,(255,255,255),-1)
        cv2.ellipse(out,(wh[0],wh[1]+int(head_r*0.1)),
                    (int(head_r*0.35),int(head_r*0.22)),0,0,180,(255,255,255),1,cv2.LINE_AA)
    return out

# ─────────────────────────────────────────────────────────────────────────────
# BUBBLES FILTER
# ─────────────────────────────────────────────────────────────────────────────

_BUBBLE_STATE: list = []

def _init_bubbles(n=60):
    rng=np.random.default_rng(int(time.time()*100)%100000)
    return [{'rx':rng.uniform(0.05,0.95),'ry':rng.uniform(0.05,0.95),
             'r':int(rng.integers(10,40)),'vx':rng.uniform(-0.002,0.002),
             'vy':rng.uniform(-0.005,-0.001),'hue':int(rng.integers(0,180)),
             'phase':rng.uniform(0,math.pi*2),'born':time.time()} for _ in range(n)]

def f_bubbles(frame, mask):
    global _BUBBLE_STATE
    h,w=frame.shape[:2]; t=time.time()
    if not _BUBBLE_STATE: _BUBBLE_STATE=_init_bubbles(70)

    hard=(mask>127).astype(np.uint8)*255
    bg_a=1.0-mask.astype(np.float32)/255.0
    tint=np.zeros_like(frame,dtype=np.float32); tint[:,:,0]=30
    bg3=np.stack([bg_a]*3,axis=2)
    bg=np.clip(frame.astype(np.float32)*bg3+tint*(1-bg3),0,255).astype(np.uint8)

    cnts,_=cv2.findContours(hard,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    all_pts=[c.reshape(-1,2) for c in cnts]
    if not all_pts: return bg
    all_pts=np.vstack(all_pts)
    gx1,gx2=int(all_pts[:,0].min()),int(all_pts[:,0].max())
    gy1,gy2=int(all_pts[:,1].min()),int(all_pts[:,1].max())
    gw=max(1,gx2-gx1); gh=max(1,gy2-gy1)

    bubble_layer=np.zeros((h,w,4),dtype=np.uint8)
    dead=[]
    rng2=np.random.default_rng(int(t*1000)%100000)

    for i,b in enumerate(_BUBBLE_STATE):
        b['ry']+=b['vy']
        b['rx']+=b['vx']+0.001*math.sin(t*1.5+b['phase'])
        if b['ry']<-0.1 or b['rx']<-0.1 or b['rx']>1.1:
            dead.append(i); continue
        bx_c=int(gx1+b['rx']*gw); by_c=int(gy1+b['ry']*gh); br=b['r']
        if 0<=bx_c<w and 0<=by_c<h and hard[by_c,bx_c]>64:
            hue=int((b['hue']+t*20)%180)
            bgr=tuple(int(v) for v in cv2.cvtColor(np.uint8([[[hue,200,230]]]),cv2.COLOR_HSV2BGR)[0][0])
            wr=max(4,int(br*(1+0.06*math.sin(t*3+b['phase']))))
            pa=int(160+40*math.sin(t*2+b['phase']))
            cv2.circle(bubble_layer,(bx_c,by_c),wr,(bgr[0],bgr[1],bgr[2],pa),-1)
            cv2.circle(bubble_layer,(bx_c,by_c),wr,(255,255,255,200),1,cv2.LINE_AA)
            cv2.circle(bubble_layer,(bx_c-wr//3,by_c-wr//3),max(2,wr//4),(255,255,255,220),-1)

    for i in sorted(dead,reverse=True):
        _BUBBLE_STATE[i]={'rx':rng2.uniform(0.05,0.95),'ry':rng2.uniform(0.6,1.1),
                          'r':int(rng2.integers(10,40)),'vx':rng2.uniform(-0.002,0.002),
                          'vy':rng2.uniform(-0.005,-0.001),'hue':int(rng2.integers(0,180)),
                          'phase':rng2.uniform(0,math.pi*2),'born':t}

    b_bgr=bubble_layer[:,:,:3].astype(np.float32)
    b_a3=np.stack([bubble_layer[:,:,3].astype(np.float32)/255.]*3,axis=2)
    mask3=np.stack([mask.astype(np.float32)/255.]*3,axis=2)
    ca=b_a3*mask3
    return np.clip(bg.astype(np.float32)*(1-ca)+b_bgr*ca,0,255).astype(np.uint8)

# ─────────────────────────────────────────────────────────────────────────────
# FILTER REGISTRY
# ─────────────────────────────────────────────────────────────────────────────

BODY_FILTERS = {
    ord('1'):("Raw",           f_raw),
    ord('2'):("Neon Outline",  f_neon_outline),
    ord('3'):("Cartoon",       f_cartoon),
    ord('4'):("Anime",         f_anime),
    ord('5'):("Pencil Sketch", f_pencil_sketch),
    ord('6'):("Cyberpunk",     f_cyberpunk),
    ord('7'):("Pixel Art",     f_pixel_art),
    ord('8'):("Oil Painting",  f_oil_paint),
    ord('9'):("Heat Vision",   f_heat_vision),
    ord('0'):("Glitch",        f_glitch),
    ord('r'):("Skeleton",      f_skeleton),
    ord('x'):("Stick Figure",  f_stick_figure),
    ord('u'):("Bubbles",       f_bubbles),
}

BODY_FILTER_LIST = [
    (ord('1'),"1","Raw",          False),
    (ord('2'),"2","Neon Outline", False),
    (ord('3'),"3","Cartoon",      False),
    (ord('4'),"4","Anime",        False),
    (ord('5'),"5","Pencil Sketch",False),
    (ord('6'),"6","Cyberpunk",    False),
    (ord('7'),"7","Pixel Art",    False),
    (ord('8'),"8","Oil Painting", False),
    (ord('9'),"9","Heat Vision",  False),
    (ord('0'),"0","Glitch",       False),
    (ord('r'),"R","Skeleton",     True),
    (ord('x'),"X","Stick Figure", True),
    (ord('u'),"U","Bubbles",      True),
]

FACE_EFFECT_LIST = [
    (ord('e'),"E","Elf Ears"),
    (ord('v'),"V","Vampire"),
    (ord('a'),"A","Angel Wings"),
    (ord('d'),"D","Demon Horns"),
    (ord('m'),"M","Mermaid"),
    (ord('f'),"F","Fairy"),
    (ord('y'),"Y","Baby Face"),
    (ord('c'),"C","Child"),
    (ord('t'),"T","Teen"),
    (ord('o'),"O","Old Age"),
    (ord('p'),"P","Age Prog"),
]

BG_LIST = [
    ("Space",  (80, 0,  100)),
    ("Forest", (20,130,  20)),
    ("Ocean",  (160,80,   0)),
    ("Sunset", (20,100, 200)),
    ("Matrix", (0, 160,   0)),
    ("None",   (60, 60,  60)),
]

# ─────────────────────────────────────────────────────────────────────────────
# BACKGROUNDS
# ─────────────────────────────────────────────────────────────────────────────

def make_space_bg(h,w):
    bg=np.zeros((h,w,3),dtype=np.uint8)
    y_idx=np.arange(h)[:,None]
    bg[:,:,2]=(60+60*y_idx/h).astype(np.uint8)
    bg[:,:,0]=(30*y_idx/h).astype(np.uint8)
    rng=np.random.default_rng(42)
    xs,ys,br=rng.integers(0,w,400),rng.integers(0,h,400),rng.integers(150,255,400)
    for x,y,b in zip(xs,ys,br): cv2.circle(bg,(int(x),int(y)),1,(int(b),int(b),int(b)),-1)
    return bg

def make_forest_bg(h,w):
    bg=np.zeros((h,w,3),dtype=np.uint8); bg[:h//2]=[180,220,120]; bg[h//2:]=[25,90,15]
    rng=np.random.default_rng(5)
    for tx in range(20,w,80):
        th=int(rng.integers(70,130))
        cv2.rectangle(bg,(tx+28,h//2-th),(tx+42,h//2+5),(35,70,15),-1)
        cv2.circle(bg,(tx+35,h//2-th),55,(15,130,25),-1)
    return bg

def make_ocean_bg(h,w):
    bg=np.zeros((h,w,3),dtype=np.uint8)
    y_idx=np.arange(h)[:,None]/h
    bg[:,:,0]=(160*(1-y_idx)).astype(np.uint8)
    bg[:,:,1]=(90+60*y_idx).astype(np.uint8)
    bg[:,:,2]=(190+65*y_idx).astype(np.uint8)
    return bg

def make_sunset_bg(h,w):
    bg=np.zeros((h,w,3),dtype=np.uint8)
    y_idx=np.arange(h)[:,None]/h
    bg[:,:,0]=(20+40*y_idx).astype(np.uint8)
    bg[:,:,1]=(60*(1-y_idx)).astype(np.uint8)
    bg[:,:,2]=(180*(1-y_idx)).astype(np.uint8)
    cv2.circle(bg,(w//2,h//2+50),75,(20,170,255),-1)
    return bg

def make_matrix_bg(h,w):
    bg=np.zeros((h,w,3),dtype=np.uint8); rng=np.random.default_rng(99); chars="01ABXYZ"
    for col_x in range(0,w,14):
        ln=int(rng.integers(5,max(6,h//14))); sy=int(rng.integers(0,h))
        for i in range(ln):
            b=max(40,255-i*20)
            cv2.putText(bg,chars[int(rng.integers(0,len(chars)))],(col_x,(sy+i*14)%h),
                        FONT,0.4,(0,b,0),1)
    return bg

_BG_CACHE = {}
BG_NAMES  = ["Space","Forest","Ocean","Sunset","Matrix"]
BG_FUNCS  = [make_space_bg,make_forest_bg,make_ocean_bg,make_sunset_bg,make_matrix_bg]

def get_bg(name,h,w):
    key=(name,h,w)
    if key not in _BG_CACHE: _BG_CACHE[key]=BG_FUNCS[BG_NAMES.index(name)](h,w)
    return _BG_CACHE[key].copy()

def composite(person,bg,mask):
    a=m3f(mask)
    return np.clip(person.astype(np.float32)*a+bg.astype(np.float32)*(1-a),0,255).astype(np.uint8)

# ─────────────────────────────────────────────────────────────────────────────
# SIDEBAR UI  (unchanged visual style)
# ─────────────────────────────────────────────────────────────────────────────

class SidebarUI:
    def __init__(self):
        self.hit_boxes: list = []

    @staticmethod
    def _rounded_rect(img,x1,y1,x2,y2,col,r=5):
        cv2.rectangle(img,(x1+r,y1),(x2-r,y2),col,-1)
        cv2.rectangle(img,(x1,y1+r),(x2,y2-r),col,-1)
        for cx,cy in [(x1+r,y1+r),(x2-r,y1+r),(x1+r,y2-r),(x2-r,y2-r)]:
            cv2.circle(img,(cx,cy),r,col,-1)

    @staticmethod
    def _gradient_rect(img,x1,y1,x2,y2,col_top,col_bot):
        rh=max(1,y2-y1)
        for dy in range(rh):
            t=dy/rh
            c=tuple(int(col_top[i]*(1-t)+col_bot[i]*t) for i in range(3))
            cv2.line(img,(x1,y1+dy),(x2,y1+dy),c,1)

    def _section_header(self,panel,y,label,icon_col=None):
        cv2.rectangle(panel,(0,y),(PANEL_W,y+SECTION_H),C_SECTION_BG,-1)
        cv2.rectangle(panel,(0,y),(3,y+SECTION_H),C_BTN_ACTIVE,-1)
        tx=8
        if icon_col:
            cv2.circle(panel,(12,y+SECTION_H//2),4,icon_col,-1)
            tx=22
        cv2.putText(panel,label,(tx,y+SECTION_H-6),FONT,FS_SEC,C_SECTION_TEXT,1,cv2.LINE_AA)
        return y+SECTION_H

    def _button(self,panel,y,label,shortcut,active,is_new=False,dot_col=None):
        x1,y1,x2,y2=PAD,y+1,PANEL_W-PAD,y+BTN_H-1
        if active:
            self._gradient_rect(panel,x1,y1,x2,y2,C_BTN_ACTIVE,C_BTN_ACTIVE2)
            cv2.rectangle(panel,(x1-1,y1-1),(x2+1,y2+1),C_BORDER_ACT,1)
            txt_col=C_ACTIVE_TEXT
        else:
            self._rounded_rect(panel,x1,y1,x2,y2,C_BTN,r=4)
            cv2.rectangle(panel,(x1,y1),(x2,y2),C_BORDER,1)
            txt_col=C_TEXT
        dot_x=x1+10; dot_y=(y1+y2)//2
        if dot_col:
            cv2.circle(panel,(dot_x,dot_y),4,(10,10,10) if active else dot_col,-1)
        badge_x=x1+22; badge_w=18; badge_y1=dot_y-8; badge_y2=dot_y+8
        self._rounded_rect(panel,badge_x,badge_y1,badge_x+badge_w,badge_y2,
                            (10,10,10) if active else (45,45,70),r=2)
        cv2.putText(panel,shortcut,(badge_x+3,badge_y1+12),FONT,0.30,(255,255,255),1,cv2.LINE_AA)
        cv2.putText(panel,label,(badge_x+badge_w+5,dot_y+5),FONT,FS,txt_col,1,cv2.LINE_AA)
        if is_new and not active:
            nx=x2-30
            self._rounded_rect(panel,nx,y1+5,nx+26,y2-5,C_NEW_BADGE,r=3)
            cv2.putText(panel,"NEW",(nx+3,y1+16),FONT,FS_BADGE,(255,255,255),1,cv2.LINE_AA)
        return y+BTN_H

    def _bg_button(self,panel,y,label,dot_col,active):
        x1,y1,x2,y2=PAD,y+1,PANEL_W-PAD,y+BTN_H-1
        if active:
            self._gradient_rect(panel,x1,y1,x2,y2,C_BTN_ACTIVE,C_BTN_ACTIVE2)
            cv2.rectangle(panel,(x1-1,y1-1),(x2+1,y2+1),C_BORDER_ACT,1)
            txt_col=C_ACTIVE_TEXT
        else:
            self._rounded_rect(panel,x1,y1,x2,y2,C_BTN,r=4)
            cv2.rectangle(panel,(x1,y1),(x2,y2),C_BORDER,1)
            txt_col=C_TEXT
        dot_x=x1+12; dot_y=(y1+y2)//2
        cv2.circle(panel,(dot_x,dot_y),7,dot_col,-1)
        cv2.circle(panel,(dot_x-2,dot_y-2),2,(220,220,220),-1)
        cv2.putText(panel,label,(x1+26,dot_y+5),FONT,FS,txt_col,1,cv2.LINE_AA)
        return y+BTN_H

    def _action_button(self,panel,y,label,col_top,col_bot=None):
        if col_bot is None: col_bot=tuple(max(0,v-40) for v in col_top)
        x1,y1,x2,y2=PAD,y+1,PANEL_W-PAD,y+BTN_H-1
        self._gradient_rect(panel,x1,y1,x2,y2,col_top,col_bot)
        cv2.rectangle(panel,(x1,y1),(x2,y2),(200,200,220),1)
        (tw,th),_=cv2.getTextSize(label,FONT,FS,1)
        cv2.putText(panel,label,((PANEL_W-tw)//2,(y1+y2+th)//2),FONT,FS,(230,230,230),1,cv2.LINE_AA)
        return y+BTN_H

    @staticmethod
    def _fps_bar(panel,y,fps,people):
        fp_col=(0,220,80) if fps>=20 else (0,180,255) if fps>=12 else (0,60,220)
        bar_w=min(PANEL_W-4,int((fps/30.)*(PANEL_W-4)))
        cv2.rectangle(panel,(2,y+2),(PANEL_W-2,y+6),(30,30,50),-1)
        cv2.rectangle(panel,(2,y+2),(2+bar_w,y+6),fp_col,-1)
        cv2.putText(panel,f"{fps:.0f} fps",(4,y+18),FONT,0.36,fp_col,1,cv2.LINE_AA)
        p_col=(0,200,255) if people>0 else (80,80,100)
        cv2.putText(panel,f"People: {people}",(PANEL_W-80,y+18),FONT,0.33,p_col,1,cv2.LINE_AA)

    def render(self,frame_h,body_key,face_key,bg_idx,show_boxes,fps,people,age_stage):
        panel=np.zeros((frame_h,PANEL_W,3),dtype=np.uint8)
        for y in range(frame_h):
            r=y/frame_h
            panel[y]=[int(14*(1-r)+10*r),int(14*(1-r)+12*r),int(22*(1-r)+18*r)]

        self.hit_boxes=[]
        self._gradient_rect(panel,0,0,PANEL_W,44,(16,16,30),(10,10,20))
        cv2.putText(panel,"FILTER",(6,20),FONT,0.55,C_ACCENT,1,cv2.LINE_AA)
        cv2.putText(panel,"CAM",(76,20),FONT,0.55,C_BTN_ACTIVE,1,cv2.LINE_AA)
        cv2.putText(panel,"v3.0",(PANEL_W-34,14),FONT,0.28,(70,70,100),1,cv2.LINE_AA)
        self._fps_bar(panel,24,fps,people)
        y=50

        y=self._section_header(panel,y,"BODY FILTER",CAT_BODY)
        for (key,sc,lbl,is_new) in BODY_FILTER_LIST:
            dot=CAT_NEW if is_new else CAT_BODY
            y0=y; y=self._button(panel,y,lbl,sc,key==body_key,is_new,dot)
            self.hit_boxes.append((y0,y,'body',key))
        y+=2

        y=self._section_header(panel,y,"FACE EFFECT",CAT_FACE)
        for (key,sc,lbl) in FACE_EFFECT_LIST:
            is_active=(key==face_key)
            display=lbl
            if key==ord('p') and is_active:
                ages=["Baby","Child","Teen","Adult","Old"]
                display=f"Age:{ages[age_stage]}"
            y0=y; y=self._button(panel,y,display,sc,is_active,False,CAT_FACE)
            self.hit_boxes.append((y0,y,'face',key))
        y+=2

        y=self._section_header(panel,y,"BACKGROUND",CAT_BG)
        for i,(name,dot_col) in enumerate(BG_LIST):
            active=(name=="None" and bg_idx==-1) or (i<len(BG_NAMES) and bg_idx==i)
            y0=y; y=self._bg_button(panel,y,name,dot_col,active)
            self.hit_boxes.append((y0,y,'bg',i if name!="None" else -1))
        y+=4

        y=self._section_header(panel,y,"ACTIONS")
        box_top=C_BOX_BTN_ON if show_boxes else C_BOX_BTN_OFF
        lbl_box="Boxes: ON" if show_boxes else "Boxes: OFF"
        y0=y; y=self._action_button(panel,y,f"[K] {lbl_box}",box_top)
        self.hit_boxes.append((y0,y,'boxes',None))
        y0=y; y=self._action_button(panel,y,"[S] Save Screenshot",(20,100,180),(10,70,130))
        self.hit_boxes.append((y0,y,'save',None))
        y0=y; y=self._action_button(panel,y,"[Q] Quit",(140,30,30),(100,20,20))
        self.hit_boxes.append((y0,y,'quit',None))

        cv2.rectangle(panel,(0,frame_h-2),(PANEL_W,frame_h),C_BTN_ACTIVE,-1)
        return panel

    def hit_test(self,x,y):
        if x<0 or x>=PANEL_W: return None
        for (y1,y2,atype,aval) in self.hit_boxes:
            if y1<=y<y2: return (atype,aval)
        return None

# ─────────────────────────────────────────────────────────────────────────────
# MOUSE STATE
# ─────────────────────────────────────────────────────────────────────────────

class MouseState:
    def __init__(self):
        self.x=0; self.y=0; self.clicked_action=None

def make_mouse_callback(ms):
    def cb(event,x,y,flags,param):
        ms.x=x; ms.y=y
        if event==cv2.EVENT_LBUTTONUP: ms.clicked_action=(x,y)
    return cb

# ─────────────────────────────────────────────────────────────────────────────
# HUD
# ─────────────────────────────────────────────────────────────────────────────

def draw_hud(frame,body_name,face_name,bg_name,fps,people):
    h,w=frame.shape[:2]
    ov=frame.copy()
    cv2.rectangle(ov,(0,0),(w,52),(0,0,0),-1)
    frame=cv2.addWeighted(ov,0.50,frame,0.50,0)
    pill=f"  {body_name}  "
    (pw,_),_=cv2.getTextSize(pill,FONT,0.52,1)
    cv2.rectangle(frame,(6,4),(pw+12,26),(0,160,100),-1)
    cv2.putText(frame,pill,(8,20),FONT,0.52,(10,10,10),1,cv2.LINE_AA)
    info=[]
    if face_name: info.append(f"Face:{face_name}")
    if bg_name:   info.append(f"BG:{bg_name}")
    info.append(f"People:{people}")
    cv2.putText(frame,"  ".join(info),(8,44),FONT,0.38,(130,130,155),1,cv2.LINE_AA)
    fps_col=(0,220,80) if fps>=20 else (0,180,255) if fps>=12 else (0,60,220)
    cv2.putText(frame,f"{fps:.0f}fps",(w-64,20),FONT,0.52,fps_col,1,cv2.LINE_AA)
    return frame

# ─────────────────────────────────────────────────────────────────────────────
# TOAST
# ─────────────────────────────────────────────────────────────────────────────

class Toast:
    def __init__(self): self.msg=""; self.until=0.; self.col=(0,200,130)
    def show(self,msg,col=(0,200,130),duration=2.0):
        self.msg=msg; self.until=time.time()+duration; self.col=col
    def draw(self,frame):
        if time.time()>self.until: return frame
        h,w=frame.shape[:2]
        (tw,th),_=cv2.getTextSize(self.msg,FONT,0.55,1)
        px,py=(w-tw)//2,h-80
        fade=min(1.0,(self.until-time.time())/0.4)
        ov=frame.copy()
        cv2.rectangle(ov,(px-12,py-th-8),(px+tw+12,py+6),(0,0,0),-1)
        cv2.rectangle(ov,(px-13,py-th-9),(px+tw+13,py+7),
                      tuple(int(v*fade) for v in self.col),2)
        frame=cv2.addWeighted(frame,1-0.7*fade,ov,0.7*fade,0)
        cv2.putText(frame,self.msg,(px,py),FONT,0.55,
                    tuple(int(v*fade) for v in self.col),1,cv2.LINE_AA)
        return frame

# ─────────────────────────────────────────────────────────────────────────────
# MAIN
# ─────────────────────────────────────────────────────────────────────────────

def main():
    cameras=find_camera()
    if not cameras:
        print("No webcam found!"); input("Press Enter..."); return

    cam_index=cameras[0]
    if len(cameras)>1:
        print(f"Multiple cameras: {cameras} — enter index [0]: ",end="",flush=True)
        try: cam_index=cameras[int(input().strip())]
        except Exception: cam_index=cameras[0]

    try:
        seg  = YOLOSegmentor("yolov8n-seg.pt")   # swap to yolov8s-seg.pt for +accuracy
        face = FaceDetector()
    except Exception as e:
        print(f"Init failed: {e}"); input("Press Enter..."); return

    cap=open_camera(cam_index)
    if not cap.isOpened():
        print("Cannot open camera."); input("Press Enter..."); return

    cap.set(cv2.CAP_PROP_FRAME_WIDTH,  1280)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
    cap.set(cv2.CAP_PROP_FPS, 30)

    body_filter_key = ord('1')
    face_effect_key = None
    bg_idx          = -1
    show_boxes      = False
    prev_time       = time.time()
    save_dir        = os.path.expanduser("~/Pictures")
    os.makedirs(save_dir, exist_ok=True)

    sidebar  = SidebarUI()
    ms       = MouseState()
    toast    = Toast()
    WIN_NAME = "Filter Cam v3"
    cv2.namedWindow(WIN_NAME, cv2.WINDOW_NORMAL)
    cv2.setMouseCallback(WIN_NAME, make_mouse_callback(ms))

    # Warm up first mask
    ret, frame = cap.read()
    if ret:
        frame = cv2.flip(frame, 1)
        seg.submit(frame)
        time.sleep(0.3)   # wait for first inference

    frame_count  = 0
    SKIP_FRAMES  = 2   # run YOLO every N frames (1=every frame, 2=every other frame)

    print("\n" + "="*58)
    print("  Filter Cam v3.0 (Optimised) — Ready!")
    print("  Threaded YOLO | Frame-skip | Mask-contour joints")
    print("  Q / Esc to quit.")
    print("="*58 + "\n")

    quit_flag = False

    while not quit_flag:
        ret, frame = cap.read()
        if not ret: continue
        frame = cv2.flip(frame, 1)
        t     = time.time()
        frame_count += 1

        # Submit to YOLO thread every SKIP_FRAMES frames
        if frame_count % SKIP_FRAMES == 0:
            seg.submit(frame)

        mask, boxes = seg.get_result()

        # Resize mask if it doesn't match frame yet (first frames)
        if mask.shape[:2] != frame.shape[:2]:
            mask = cv2.resize(mask, (frame.shape[1], frame.shape[0]))

        # Body filter
        b_name, b_fn = BODY_FILTERS[body_filter_key]
        filtered = b_fn(frame, mask)

        # Background
        if bg_idx >= 0:
            h_f, w_f = frame.shape[:2]
            filtered  = composite(filtered, get_bg(BG_NAMES[bg_idx],h_f,w_f), mask)

        # Bounding boxes
        if show_boxes:
            for i,box in enumerate(boxes):
                x1,y1,x2,y2=box
                cv2.rectangle(filtered,(x1,y1),(x2,y2),(0,220,140),2)
                cv2.putText(filtered,f"Person {i+1}",(x1+4,y1-6),FONT,0.48,(255,255,255),1)

        # Face effect
        f_name = None
        if face_effect_key is not None:
            faces=face.detect(frame)
            f_name,f_fn=FACE_EFFECTS[face_effect_key]
            filtered=f_fn(filtered,faces,t) if face_effect_key!=ord('p') \
                     else effect_age_progression(filtered,faces,t)

        # HUD
        fps=1./(max(t-prev_time,1e-6)); prev_time=t
        bg_name=BG_NAMES[bg_idx] if bg_idx>=0 else None
        filtered=draw_hud(filtered,b_name,f_name,bg_name,fps,len(boxes))
        filtered=toast.draw(filtered)

        # Sidebar
        h_f,w_f=filtered.shape[:2]
        panel=sidebar.render(h_f,body_filter_key,face_effect_key,
                             bg_idx,show_boxes,fps,len(boxes),AGE_PROGRESSION_STAGE[0])
        display=np.hstack([panel,filtered])
        cv2.imshow(WIN_NAME,display)

        # Mouse
        if ms.clicked_action is not None:
            cx,cy=ms.clicked_action; ms.clicked_action=None
            hit=sidebar.hit_test(cx,cy)
            if hit:
                atype,aval=hit
                if atype=='body':
                    body_filter_key=aval; toast.show(f"Filter: {BODY_FILTERS[aval][0]}")
                elif atype=='face':
                    if face_effect_key==aval:
                        if aval==ord('p'):
                            AGE_PROGRESSION_STAGE[0]=(AGE_PROGRESSION_STAGE[0]+1)%5
                            ages=["Baby","Child","Teen","Adult","Elderly"]
                            toast.show(f"Age: {ages[AGE_PROGRESSION_STAGE[0]]}")
                        else:
                            face_effect_key=None; toast.show("Face effect: Off",(140,140,160))
                    else:
                        face_effect_key=aval
                        if aval==ord('p'): AGE_PROGRESSION_STAGE[0]=0
                        toast.show(f"Effect: {FACE_EFFECTS[aval][0]}",(200,80,220))
                elif atype=='bg':
                    bg_idx=aval
                    name=BG_NAMES[bg_idx] if bg_idx>=0 else "None"
                    toast.show(f"Background: {name}",(220,140,20))
                elif atype=='boxes':
                    show_boxes=not show_boxes
                    toast.show(f"Boxes: {'ON' if show_boxes else 'OFF'}",(20,130,20))
                elif atype=='save':
                    ts=time.strftime("%Y%m%d_%H%M%S")
                    path=os.path.join(save_dir,f"filterCam_{ts}.png")
                    cv2.imwrite(path,display)
                    toast.show("Saved!",(20,120,200)); print(f"Saved → {path}")
                elif atype=='quit':
                    quit_flag=True

        # Keyboard
        key=cv2.waitKey(1)&0xFF
        if key in (ord('q'),27): break
        elif key in BODY_FILTERS:
            body_filter_key=key; toast.show(f"Filter: {BODY_FILTERS[key][0]}")
        elif key in FACE_EFFECTS:
            if face_effect_key==key:
                face_effect_key=None if key!=ord('p') else face_effect_key
                if key==ord('p'): AGE_PROGRESSION_STAGE[0]=(AGE_PROGRESSION_STAGE[0]+1)%5
            else:
                face_effect_key=key
                if key==ord('p'): AGE_PROGRESSION_STAGE[0]=0
        elif key==ord('b'):
            bg_idx=(bg_idx+1)%len(BG_NAMES)
            toast.show(f"Background: {BG_NAMES[bg_idx]}",(220,140,20))
        elif key==ord('n'):
            bg_idx=-1; toast.show("Background: Off",(140,140,160))
        elif key==ord('k'):
            show_boxes=not show_boxes
        elif key==ord('s'):
            ts=time.strftime("%Y%m%d_%H%M%S")
            path=os.path.join(save_dir,f"filterCam_{ts}.png")
            cv2.imwrite(path,display)
            toast.show("Saved!",(20,120,200)); print(f"Saved → {path}")

    seg.stop()
    cap.release()
    cv2.destroyAllWindows()
    print("Bye!")

if __name__=="__main__":
    main()